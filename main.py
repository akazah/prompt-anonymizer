...
class PromptAnonymizer:
    ...
    def _get_messages(self, language: str) -> Dict[str, str]:
        ...
        if language == "ja":
            ...
            return {
                ...
                "prompt_send": (
                    f"{Fore.GREEN}Do you send this anonymized text as a prompt to OpenAI?\n"
                    f"(Anonymization can fail. Please check carefully){Style.RESET_ALL}"
                ),
                "input_prompt": "(n)o,(Y)es > ",
    ...
    def _display_anonymization_result(
        self, original_text: str, anonymized_texts: Dict[str, str], messages: Dict[str, str]
    ) -> None:
        """
    ...
    def _send_to_openai(
        self, anonymized_text: str, messages: Dict[str, str]
    ) -> str:
        """
