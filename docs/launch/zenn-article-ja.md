...
日本語・英語・スペイン語・ベトナム語を**対等に**扱い、それぞれに
ロケール固有の構造化 PII を用意しています(`-l ja|en|es|vi`、自動判定あり)。
...
```bash
pip install prompt-anonymizer   # ← 公開後。それまでは pip install git+https://github.com/akazah/prompt-anonymizer
...
```bash
npx @prompt-anonymizer/proxy        # ← 公開後
