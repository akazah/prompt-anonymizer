...
| [stdnum](https://www.npmjs.com/package/stdnum)（npm、python-stdnumのTS移植） | MIT | 採用可（ただし汎用IBANモジュールが無い。国別IDで必要になった時点で導入） |
| [ibantools](https://www.npmjs.com/package/ibantools)（npm、IBAN mod-97検証） | MIT | 採用可（P16で採用済み。stdnum npmに汎用IBAN検証が無いため） |
| [gliner](https://pypi.org/project/gliner/)（PyPI、GLiNERランタイム） | Apache-2.0 | 採用可（P17で `gliner` extra として opt-in 導入） |
| [janome](https://pypi.org/project/janome/)（PyPI、ja GLiNER 用形態素分割） | Apache-2.0 | 採用可（`gliner` extra のみ。`hf` / `spacy` では未使用） |
| [gliner](https://www.npmjs.com/package/gliner)（npm、GLiNER.js） | MIT | 採用可（メンテ状況に留意、P17参照） |
| libphonenumber-js（必要時のみ） | MIT | 採用可 |
| モデル: `urchade/gliner_multi_pii-v1`（en/fr/de/es/it/pt） | Apache-2.0 | 採用可（P17 opt-in。リビジョン `1fcf13e…`） |
| モデル: `DataSign/gliner-ja-pii-v1`（ja） | Apache-2.0 | 採用可（P17 opt-in。リビジョン `54e97b3…`） |
| ベースモデル: `urchade/gliner_multi-v2.1`（multi PII の親） | Apache-2.0 | 採用可（直接は未使用。由来チェーン記録用） |
...
| データセット: `urchade/synthetic-pii-ner-mistral-v1` | Apache-2.0 | 採用可（Mistral 生成の合成 PII。multi PII の学習元） |
...
2. 検出正規表現は `recognizers.ts` の既存流儀（有界量指定子・lookaround・
   `(?<![\d-])` 系の境界）で自前実装し、`validate` に検証ライブラリを接続する。
   ReDoSレビュー（AGENTS.md P1）を必須とする。
   注: IBANについては stdnum（npm）に汎用IBANモジュールが存在しないため、
   ibantools（MIT・依存ゼロ）を採用する。stdnum は国別ID（チェックディジット
   付き）を追加する段階で導入する
...
Python側の評価を実施した（`ner_backend="gliner"`、`gliner` extra、
モデルは `recognizers/gliner_ner.py` でリビジョンピン）。golden set
（200件/言語）での結果は `docs/EVAL.md` のバックエンド比較表を参照。要点:

1. **精度**: gliner は golden set の PERSON / LOCATION 全セルで hf 以上。
   主な改善は ja LOCATION F1 0.94 → 1.00（誤検出の大幅減）と
   en PERSON 0.95 → 1.00。es / vi は spaCy とのunion由来の誤検出が支配的で
   ほぼ変化なし
2. **サイズ**: ja モデル（`DataSign/gliner-ja-pii-v1`）は約1.1GB
   （fp32 PyTorch）。hf バックエンドの xlm-roberta 系と同等であり、
   Python側では追加負担にならない
3. **レイテンシ**: CPU実測（warm、50件中央値）で ja 約135ms/件。
   hf（約46ms）の約3倍、spacy（約9ms）の約15倍。対話的用途では許容内

**判断: Python側 opt-in 採用**（既定バックエンドは `spacy` のまま変更しない。
既定変更は初回ダウンロード1.1GBを全ユーザーに課すため見送り）。
精度最優先のユーザー向けに `pip install "prompt-anonymizer[gliner]"` +
`--ner-backend gliner` を提供し、weekly CI（`weekly-eval.yml` の
`eval-gliner` ジョブ、モデルはリビジョンキーでキャッシュ）で回帰追跡する。

**TS側: 現行モデル据え置き（非対称構成）**。GLiNER の ONNX 公式 q8
エクスポートが両モデルとも存在せず、`gliner`（npm）は onnxruntime-web の
二重ランタイムを抱えるうえメンテ頻度が低い。golden set 上の改善幅
（hf 比で数ポイント）はブラウザ向けバンドル・ダウンロード増を正当化しない。
ONNX エクスポートの整備状況とモデルの更新を weekly 評価で再判断する。

採用モデルの監査記録（ライセンス方針に基づくリビジョンピン。ピンの実体は
`src/prompt_anonymizer/recognizers/gliner_ner.py`）:

| モデル | リビジョン | ライセンス | 取得日 | 学習データ |
|---|---|---|---|---|
| `urchade/gliner_multi_pii-v1` | `1fcf13e85f4eef5394e1fcd406cf2ca9ea82351d` | Apache-2.0 | 2026-07-19 | `urchade/synthetic-pii-ner-mistral-v1`（合成） |
| `DataSign/gliner-ja-pii-v1` | `54e97b315fc6d604847c8d273e0829794260540d` | Apache-2.0 | 2026-07-19 | Faker ja_JP 合成データ（実データなし） |

#### P17 ライセンス審査（2026-07-19）

上記「ライセンス方針」の3条件に対する審査結果。結論: **両モデルとも opt-in
採用可**。既定バックエンドは変更せず、ウェイトの再配布・永続化も行わない。

**受け入れ基準チェック**

| 条件 | `urchade/gliner_multi_pii-v1` | `DataSign/gliner-ja-pii-v1` |
|---|---|---|
| (1) 商用利用可 | ✓ Apache-2.0 | ✓ Apache-2.0 |
| (2) 派生物作成可（ONNX 変換・量子化等） | ✓ | ✓ |
| (3) 学習データによる汚染なし | ✓ 下記由来チェーン | ✓ Faker（MIT）合成のみ |

**コード依存（`pip install "prompt-anonymizer[gliner]"` 時のみ）**

| パッケージ | ライセンス | 導入経路 | 判定 |
|---|---|---|---|
| `gliner` | Apache-2.0 | 直接 | 採用可 |
| `janome` | Apache-2.0 | 直接（ja 用 word splitter） | 採用可 |
| `torch` | BSD-3-Clause | `gliner` 推移 | 採用可 |
| `transformers` | Apache-2.0 | `gliner` 推移 | 採用可 |
| `onnxruntime` | MIT | `gliner` 推移 | 採用可 |
| `sentencepiece` | Apache-2.0 | `gliner` 推移 | 採用可 |

いずれも MIT / Apache-2.0 / BSD 系。LGPL / GPL / 独自二段階ライセンスは含まれない。
`gliner` extra を入れない限り `torch` / `transformers` は `hf` extra 経由でのみ
入る（`hf` も opt-in）。本体の MIT 依存ツリーは変わらない。

**モデル・学習データの由来チェーン**

```
urchade/gliner_multi-v2.1          Apache-2.0  (GLiNER 汎用ベース)
  └─ fine-tune ─ urchade/synthetic-pii-ner-mistral-v1  Apache-2.0
                 (Mistral 生成の合成 PII NER データ。実 PII なし)
       └─ urchade/gliner_multi_pii-v1  Apache-2.0  ← en 等で使用（リビジョンピン）
            └─ fine-tune ─ Faker ja_JP 合成テンプレート  MIT
                           (実データなし。モデルカード明記)
                 └─ DataSign/gliner-ja-pii-v1  Apache-2.0  ← ja で使用（リビジョンピン）
```

- **multi PII**: ベース `gliner_multi-v2.1` と学習データ `synthetic-pii-ner-mistral-v1`
  はいずれも Hugging Face 上で Apache-2.0。ai4privacy 系の二段階ライセンスデータは
  不使用（Piiranha 不採用理由と同根の汚染リスクを回避）。
- **ja PII**: multi PII 上に Faker 合成のみで追加学習。Faker は MIT。
  ベースモデルの Apache-2.0 条件（派生物のライセンス継承）を満たす fine-tune。

**PII 漏えい・再配布（P0 との関係）**

- ウェイトは Hugging Face CDN から実行時に端末へダウンロードするが、
  マッピング同様にリポジトリ同梱・ログ出力・永続化はしない。
- 入力テキストをモデル提供者へ送ることはない（完全オンデバイス推論）。
- `scan` / MCP の P0 契約（マッチ文字列を外部へ出さない）は GLiNER 利用時も不変。

**利用者への義務**

- Apache-2.0 ウェイトを**再配布しない**通常の推論利用では、エンドユーザーに
  NOTICE ファイルの同梱義務は生じない（当ツールはウェイトを同梱しない）。
- 出力テキストへの帰属表示義務もなし（CC-BY 系データセット由来ではない）。
- 年商制限・NC（非商用）・ND（派生禁止）条項はいずれの依存にもない。

**残存リスクと緩和策**

| リスク | 影響 | 緩和策 |
|---|---|---|
| モデルカードのライセンス変更 | 商用利用者の法的リスク | リビジョンピン（`gliner_ner.py`）。更新時は再審査必須。Piiranha 先例を監査表に記録済み |
| Mistral 生成合成データの upstream 規約 | データセット配布者の責任範囲のグレーゾーン | 当ツールはデータセットを再配布しない。採用は HF 上の Apache-2.0 表記に依拠 |
| ja モデルの合成データのみ学習 | 精度限界（ライセンスとは別問題） | モデルカードの制限事項を監査記録に明記。実データ fine-tune は別モデルとして再審査 |
| `gliner` 推移依存の増加 | サプライチェーン監査コスト | opt-in extra に限定。`uv.lock` でバージョン固定。更新時は推移依存を再確認 |

**不採用モデルとの対比（再確認）**

| モデル | 不採用理由 | GLiNER 採用モデルとの差 |
|---|---|---|
| Piiranha (`CC-BY-NC-ND-4.0`) | 商用不可・派生（量子化）不可 | Apache-2.0。ONNX 変換の余地あり（今回は未使用） |
| LFM2-350M-PII (`LFM Open License`) | 年商 $10M 超で商用不可 | 利用者への制約転嫁なし |
| ai4privacy pii-masking-200k/400k | 企業向け有償の二段階 | 合成 Apache-2.0 / Faker MIT のみ |
