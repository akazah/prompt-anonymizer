...
> **프롬프트에 대한 더블 체크 — LLM에 닿기 전에.**
> 되돌릴 수 있는 온디바이스 익명화가, 보낼 생각이 없던 PII를 잡아냅니다.
...
당신의 팀에는 이미 규칙이 있습니다: *고객 데이터나 기밀, 개인정보를
ChatGPT / Claude / Gemini에 붙여넣지 말 것.* 하지만 사람은 늘 바쁘고, 이름이나
전화번호가 슬쩍 끼어듭니다. Prompt Anonymizer가 바로 그 더블 체크입니다 —
**당신의 컴퓨터에서** 실행되어 텍스트가 떠나기 **전에** 그 PII를 잡아내므로,
실수가 유출로 이어지지 않습니다. 규칙이나 당신의 판단을 대체하는 것이 아니라,
그것을 뒷받침합니다.

| PII 실수에 대한 방어선 | 잡아내는가? | 신뢰해야 하는 것 |
|---|---|---|
| 문서로 된 규칙만 | ✗ 기억에 의존 | 모두가, 매번 |
| 전송 직전 자신의 주의력 | ~ 기억날 때만 | 바쁜 와중의 주의력 |
| **+ Prompt Anonymizer** | **✓ 자동, 온디바이스** | **읽을 수 있는 코드** |

텍스트가 컴퓨터를 떠나기 **전에** PII를 일관된 레이블(`<人名_1>`, `<Name_1>`,
`<Nombre_1>`, `<Tên_1>` 등)로 치환합니다. 같은 값에는 항상 같은 레이블이
붙기 때문에 LLM의 답변은 여전히 문맥이 통합니다 — 안전을 위해 프런티어급
지능을 포기할 필요가 없습니다. 응답이 돌아오면, 기기를 한 번도 떠난 적 없는
매핑이 실제 값을 복원합니다.

사람에게는 브라우저, 데스크톱 앱, Chrome 확장에서의 '또 하나의 눈'입니다.
파이프라인에는 같은 검사가 자동으로 적용됩니다 — OpenAI 호환 프록시가 전송
전에 마스킹하고, `scan` 게이트가 PII가 섞인 커밋이나 CI 실행을 실패시킵니다.
...
(`languages.py` / `languages.ts`)와 레이블 파일 하나뿐입니다.
...
<img alt="브라우저 앱 데모: 익명화, 매핑, 복원 왕복" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_web_ko.gif?raw=true" width="85%">

<details>
<summary>CLI 데모</summary>

<img alt="CLI 데모" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_ko.gif?raw=true" width="70%">
</details>

<details>
<summary>Chrome 확장 프로그램 데모 (사이드 패널)</summary>

<img alt="Chrome 확장 프로그램 데모" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_extension_ko.gif?raw=true" width="40%">
</details>
...
| **Python / CLI** | `pip install prompt-anonymizer` | Presidio + spaCy. 아래 빠른 시작 참고. |
| **Node CLI (npx)** | `npx @prompt-anonymizer/cli` | Python CLI와 동일한 명령·플래그. transformers.js NER, 완전 온디바이스. |
| **Web Component** | `@prompt-anonymizer/element` | 프레임워크에 독립적인 `<prompt-anonymizer>` 요소: 익명화 → 복원 패널 전체를 어떤 사이트에도 삽입할 수 있습니다(순수 HTML, Svelte, Angular 등). |
| **React / Vue** | `@prompt-anonymizer/react` / `@prompt-anonymizer/vue` | 바로 쓸 수 있는 `<AnonymizerPanel />` 컴포넌트와 커스텀 UI용 `useAnonymizer()` 훅 / 컴포저블. 아래 빠른 시작 참고. |
| **로컬 프록시 + 관리 GUI** | `npx @prompt-anonymizer/proxy` | OpenAI 호환 리버스 프록시: `OPENAI_BASE_URL`을 이쪽으로 지정하면 PII가 컴퓨터를 떠나기 전에 마스킹되고, 응답의 레이블은 복원됩니다(스트리밍 포함). 관리 GUI는 `http://127.0.0.1:8787/admin/`. 아래 빠른 시작 참고. |
| **MCP 서버** | `npx @prompt-anonymizer/mcp` | 모든 MCP 클라이언트(Claude Desktop, Claude Code, Cursor 등)용 `anonymize` / `deanonymize` / `scan` 도구. 레이블 매핑은 서버 메모리에 유지(`mapping_id`)되며, 명시적으로 요청하지 않는 한 모델에 표시되지 않습니다. 아래 빠른 시작 참고. |
...
```bash
npx @prompt-anonymizer/proxy            # http://127.0.0.1:8787 에서 수신
...
Claude Desktop, Claude Code, Cursor 등 모든 MCP 클라이언트에 온디바이스
익명화 도구를 제공합니다:
...
세 가지 도구는 모두 PII가 모델 컨텍스트에 들어가지 않도록 설계되었습니다.
`anonymize`는 마스킹된 텍스트와 `mapping_id`를 반환하고(매핑은 명시적으로
요청하지 않는 한 서버 메모리에 유지), `deanonymize`는 `mapping_id`로 복원하며
— 선택적으로 파일에 바로 쓸 수 있습니다 — `scan`은 파일의 PII를 검사해
`file:line:col`과 엔티티 유형만 보고하며 매칭된 텍스트는 절대 출력하지 않습니다.
서버 인자에 `--ner`를 넘기면 이름/위치도 마스킹합니다(최초 사용 시 모델을 한 번
다운로드).
...
PyPI / npm 레지스트리 게시(Trusted Publishing 활성화 — 현재는 GitHub
Releases에서 설치 가능), Chrome Web Store 등록, 코드 서명, 더 작은 일본어
NER 모델, 다지역 구조화 PII(체크섬 검증을 통한 전화번호 / 국민 ID 형식
추가).

## Contributing / Security / License

- [docs/INTEGRATIONS.md](../INTEGRATIONS.md) — LiteLLM, OpenWebUI, MCP 클라이언트, git 훅 및 CI 레시피
- [CONTRIBUTING.md](../../.github/CONTRIBUTING.md) — 개발 환경 설정(uv / pnpm), 테스트·평가 명령
- [docs/AUDIT.md](../AUDIT.md) — 온디바이스 주장을 직접 검증하는 절차
