...
Claude Desktop, Claude Code, Cursor 등 모든 MCP 클라이언트에
온디바이스 익명화 도구를 추가할 수 있습니다:
...
세 도구 모두 PII가 모델 컨텍스트에 들어가지 않도록 설계되었습니다.
`anonymize`는 마스킹된 텍스트와 `mapping_id`를 반환하고(매핑은 명시적
요청이 없으면 서버 메모리에만 유지), `deanonymize`는 `mapping_id`로
복원(파일로 직접 쓰기 가능)하며, `scan`은 파일의 PII를 검사해
`file:line:col`과 유형만 보고합니다(매칭 텍스트는 출력하지 않음).
서버 인자에 `--ner`를 추가하면 이름 / 위치도 마스킹합니다(최초 1회
모델 다운로드).
