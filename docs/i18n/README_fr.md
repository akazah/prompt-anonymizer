...
> **Un second regard sur vos prompts, avant qu'ils n'atteignent un LLM.**
> Anonymisation réversible, sur l'appareil, qui rattrape les PII que vous n'aviez pas l'intention d'envoyer.
...
Votre équipe a déjà la règle : *ne collez pas de données clients, de secrets
ni d'informations personnelles dans ChatGPT / Claude / Gemini.* Mais on est
pressé, et un nom ou un numéro de téléphone se glisse. Prompt Anonymizer est
ce second regard — il s'exécute **sur votre machine** et rattrape ces PII
**avant** que le texte ne parte, pour qu'une étourderie ne devienne pas une
fuite. Il ne remplace ni la règle ni votre jugement ; il les épaule.

| Ligne de défense face à une étourderie PII | La rattrape ? | Ce à quoi vous faites confiance |
|---|---|---|
| Une politique écrite, seule | ✗ repose sur la mémoire | tout le monde, à chaque fois |
| Votre propre vigilance de dernière seconde | ~ quand vous y pensez | votre attention, dans la précipitation |
| **+ Prompt Anonymizer** | **✓ automatique, sur l'appareil** | **du code que vous pouvez lire** |
...
reste cohérente — vous ne renoncez pas à l'intelligence de pointe pour rester
en sécurité. Quand la réponse revient, le mapping — qui n'a jamais quitté
votre appareil — restaure les valeurs réelles.

Pour les personnes, c'est une seconde paire d'yeux dans le navigateur,
l'application de bureau ou l'extension Chrome. Pour les pipelines, la même
vérification s'applique automatiquement — le proxy compatible OpenAI masque
avant l'envoi, et la porte `scan` fait échouer un commit ou une exécution CI
quand un PII se glisse.
...
saillants : Chrome Web Store, signature du code,
