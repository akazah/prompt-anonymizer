...
> **Das Vier-Augen-Prinzip für Ihre Prompts – bevor sie ein LLM erreichen.**
> Reversible Anonymisierung direkt auf dem Gerät, die PII abfängt, die Sie gar nicht senden wollten.
...
Ihr Team hat bereits die Regel: *keine Kundendaten, Geheimnisse oder
persönlichen Informationen in ChatGPT / Claude / Gemini einfügen.* Aber man
ist in Eile, und ein Name oder eine Telefonnummer rutscht durch. Prompt
Anonymizer ist dieses zweite Augenpaar – es läuft **auf Ihrem Rechner** und
fängt diese PII ab, **bevor** der Text ihn verlässt, damit aus einem
Ausrutscher kein Leck wird. Es ersetzt weder die Regel noch Ihr Urteil; es
stützt sie.

| Verteidigungslinie gegen einen PII-Ausrutscher | Fängt sie es ab? | Worauf Sie vertrauen |
|---|---|---|
| Eine schriftliche Richtlinie allein | ✗ verlässt sich aufs Gedächtnis | auf jeden, jedes Mal |
| Ihre eigene Wachsamkeit in letzter Sekunde | ~ wenn Sie daran denken | auf Ihre Aufmerksamkeit, mitten in der Hektik |
| **+ Prompt Anonymizer** | **✓ automatisch, auf dem Gerät** | **auf Code, den Sie lesen können** |
...
weiterhin Sinn — Sie geben für Ihre Sicherheit keine Frontier-Intelligenz
auf. Kommt die Antwort zurück, stellt das Mapping — das Ihr Gerät nie
verlassen hat — die echten Werte wieder her.

Für Menschen ist es ein zweites Augenpaar im Browser, in der Desktop-App oder
der Chrome-Erweiterung. Für Pipelines wird dieselbe Prüfung automatisch
erzwungen — der OpenAI-kompatible Proxy maskiert vor dem Versand, und das
`scan`-Gate lässt einen Commit oder CI-Lauf scheitern, wenn PII durchrutscht.
