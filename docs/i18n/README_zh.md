...
语言只需一条注册表记录（`languages.py` / `types.ts`）加一个标签文件。
...
<img alt="浏览器应用演示：匿名化、映射表、还原的完整往返" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_web.gif?raw=true" width="85%">

<details>
<summary>CLI 演示（日语 / 英语 — 其余八种语言用法相同）</summary>

<img alt="CLI 演示（日语）" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_ja.gif?raw=true" width="49%"> <img alt="CLI 演示（英语）" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_en.gif?raw=true" width="49%">
</details>

<details>
<summary>Chrome 扩展演示（侧边栏）</summary>

<img alt="Chrome 扩展演示" src="https://github.com/akazah/prompt-anonymizer/blob/main/demo/demo_extension.gif?raw=true" width="40%">
</details>
...
| **Python / CLI** | `pip install git+https://github.com/akazah/prompt-anonymizer`（尚未发布到 PyPI） | Presidio + spaCy。参见下方快速上手。 |
| **Node CLI（npx）** | `npx @prompt-anonymizer/cli`（尚未发布到 npm — 从 `web/packages/cli` 构建） | 与 Python CLI 相同的命令和参数；transformers.js NER，完全设备端。 |
| **Web Component** | `@prompt-anonymizer/element`（尚未发布到 npm） | 与框架无关的 `<prompt-anonymizer>` 元素：把完整的匿名化 → 还原面板嵌入任意站点（纯 HTML、Svelte、Angular 等）。 |
| **React / Vue** | `@prompt-anonymizer/react` / `@prompt-anonymizer/vue`（尚未发布到 npm） | 开箱即用的 `<AnonymizerPanel />` 组件，外加面向自定义 UI 的 `useAnonymizer()` hook / 组合式函数。参见下方快速上手。 |
| **本地代理 + 管理 GUI** | `@prompt-anonymizer/proxy`（尚未发布到 npm — 从 `web/packages/proxy` 构建） | OpenAI 兼容的反向代理：把 `OPENAI_BASE_URL` 指向它，PII 在离开你的机器前被遮蔽，响应中的标签被还原（含流式）。管理 GUI 位于 `http://127.0.0.1:8787/admin/`。参见下方快速上手。 |
...
# 发布后: npx @prompt-anonymizer/cli anonymize -t "..."
```
...
node packages/proxy/dist/cli.js            # 监听 http://127.0.0.1:8787
# 发布后: npx @prompt-anonymizer/proxy
...
（发布后用 `npx @prompt-anonymizer/cli`；在此之前从
`web/packages/cli` 构建）：
...
[docs/EVAL.md](docs/EVAL.md)，用 `uv run python -m prompt_anonymizer.evals`
...
[docs/EVAL.md](docs/EVAL.md) 有 TS 核心的表格；Python 的 NER 数值由每周
eval 生成。
...
[IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md)。重点：npm / PyPI 发布、
商店上架（Chrome Web Store）、代码签名、更小的日语 NER 模型、多地区
结构化 PII（通过校验和验证支持更多电话 / 国民 ID 格式）、MCP 服务器。

## Contributing / Security / License

- [CONTRIBUTING.md](CONTRIBUTING.md) — 开发环境（uv / pnpm）、测试与评估命令
- [SECURITY.md](SECURITY.md) — 报告漏洞与匿名化绕过
- [MIT](LICENSE)
