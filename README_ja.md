...
| **React / Vue** | `@prompt-anonymizer/react` / `@prompt-anonymizer/vue`（npm未公開） | 共有TSコアを包む `useAnonymizer()` フック / コンポーザブル。下のQuickstart参照。 |
...
React（`@prompt-anonymizer/react`）/ Vue 3（`@prompt-anonymizer/vue`）
バインディングは「匿名化 → LLM → 復元」のセッションをフック /
コンポーザブルとして提供します:
