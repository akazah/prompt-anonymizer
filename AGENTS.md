...
- TypeScript: `web/packages/core/` (regex + transformers.js NER) — browser app, Chrome extension, Tauri desktop, Node CLI (`web/packages/cli`), embeddable web component (`web/packages/element`), React/Vue bindings (`web/packages/{react,vue}`), OpenAI-compatible anonymizing proxy + admin GUI (`web/packages/proxy`, `web/apps/proxy-admin`), MCP server (`web/packages/mcp`; the mapping stays server-side behind a `mapping_id` — never returned to the model unless explicitly requested, P0)
