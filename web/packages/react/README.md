# @prompt-anonymizer/react

React bindings for [prompt-anonymizer](https://github.com/akazah/prompt-anonymizer):
...
```tsx
import { AnonymizerPanel } from "@prompt-anonymizer/react";
...
Hook for custom UIs — the full anonymize → LLM → restore session:

```tsx
import { useAnonymizer } from "@prompt-anonymizer/react";
